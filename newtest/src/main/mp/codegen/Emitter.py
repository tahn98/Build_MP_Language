...
class Emitter():
    ...
    *   @param in the code to be printed out
    ...
    def emitAndThen_OrElse(self,op,left,right,frame):
        #op:    String
        #left:  String
        #right: string
        #frame: Frame
        ...
        result = []
        labelexitcompare = frame.getNewLabel()
        labelgoon = frame.getNewLabel()

        if op == "andthen":
            result.append(left)
            result.append(self.jvm.emitIFLE(labelexitcompare))
            frame.pop()
            result.append(right)
            result.append(self.jvm.emitIFLE(labelexitcompare))
            frame.pop()
            result.append(self.emitPUSHCONST(1, IntType(), frame))
            frame.pop()
            result.append(self.emitGOTO(labelgoon,frame))
            result.append(self.emitLABEL(labelexitcompare,frame))
            result.append(self.emitPUSHCONST(0, IntType(), frame))
            result.append(self.emitLABEL(labelgoon,frame))        

        elif op == "orelse":
            result.append(left)
            result.append(self.jvm.emitIFGT(labelexitcompare))
            frame.pop()
            result.append(right)
            result.append(self.jvm.emitIFGT(labelexitcompare))
            frame.pop()
